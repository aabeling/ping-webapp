...
preferencesPanel.port.on("refresh-rate-changed", function(value) {
	console.log("new refresh rate received: " + value);
	ss.storage.refreshRate = value;
	updateTimer();
});
...
updateTimer = function() {
	...
	console.log("starting timer with refresh rate: " + ss.storage.refreshRate + "s");
	timerId = timers.setInterval(pingUrls, ss.storage.refreshRate * 1000);
};
